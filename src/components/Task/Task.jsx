...
const Task = ({ task, changeStatus, deleteTask, edit }) => {
  ...
  return (
    <li className={`${task.completed ? 'completed' : ''} ${task.editing ? 'editing' : ''}`}>
      ...
      <input onChange={e => setEditingTask({ ...editingTask, description: e.target.value })} ref={editInput} type="text" className="edit" value={editingTask.description} onBlur={() => edit(editingTask)} />
    </li>
  );
};
